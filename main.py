...
face_data = pd.DataFrame(columns=["Name", "Timestamp", "BookID"])
...
collection = db['face_data_with_book']
...
#            subprocess.run("C:\\Users\\Public\\lalitha2\\barcodeRunner.bat" ,  shell=True )
            scanner()
            data = {"Name": name, "Timestamp": timestamp, "barcode": y}
            face_data_list.append(data)
            xstat =collection.insert_one(data)
            if xstat is True:
                print("hello harinadh ")
        elif name == "Unknown":
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
